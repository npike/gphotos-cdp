...
// viewed item.
func requestDownloadBackup(ctx context.Context, log zerolog.Logger, imageId string) error {
...
func (s *Session) startDownload(ctx context.Context, log zerolog.Logger, imageId string, isOriginal bool, hasOriginal *bool, downloadChan <-chan NewDownload) (newDownload NewDownload, progressChan <-chan bool, err error) {
	...
	for {
		...
		case <-timeoutTimer.C:
			return NewDownload{}, nil, fmt.Errorf("timeout waiting for download to start for %v (%s)", imageId, s.getPhotoUrl(imageId))
